...
!developmentChains.includes(network.name)
    ...
    : describe("Staking Unit Tests", function () {
          ...
          beforeEach(async () => {
              ...
              deployer = accounts[0]
              await deployments.fixture(["mocks", "rewardToken", "staking", stakingToken])
          ...
          describe("constructor", () => {
              it("sets the rewards token address correctly", async () => {
                  const response = await staking.rewardToken()
                  assert.equal(response, rewardToken.address)
              })

              it("set the rewards token address correctly", async() =>{
                const response = await staking.stakingToken()
                assert.equal(reponse,stakingToken.address)
              } )
          })
